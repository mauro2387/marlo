...
export default function ConfiguracionPage() {
    ...
    try {
      ...
      if (settingsRes.data) {
        ...
        if (settingsRes.data.blocked_delivery_days && settingsRes.data.blocked_delivery_days.length > 0) {
          const saved = settingsRes.data.blocked_delivery_days;
          // Si la DB tiene los 7 días, usarlos directamente; si no, mergear con el template
          if (saved.length === 7) {
            setBlockedDeliveryDays(saved);
          } else {
            // Mergear: la DB puede tener items parciales, mantener el template y aplicar blocked/reason
            const dayNames = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'];
            const merged = dayNames.map((name, idx) => {
              const fromDb = saved.find((d: any) => d.day_index === idx);
              return fromDb || { day_index: idx, day_name: name, blocked: false };
            });
            setBlockedDeliveryDays(merged);
          }
        }
  ...
  const handleToggleBlockedDay = (dayIndex: number) => {
    setBlockedDeliveryDays(prev => {
      const updated = prev.map(day => 
        day.day_index === dayIndex ? { ...day, blocked: !day.blocked } : day
      );
      // Auto-actualizar el mensaje de la leyenda con los días bloqueados
      const blockedNames = updated.filter(d => d.blocked).map(d => d.day_name);
      if (blockedNames.length > 0) {
        const daysList = blockedNames.length === 1 
          ? `los ${blockedNames[0].toLowerCase()}`
          : blockedNames.slice(0, -1).map(n => n.toLowerCase()).join(', ') + ' y ' + blockedNames[blockedNames.length - 1].toLowerCase();
        setDeliveryNotice(prev => ({ ...prev, message: `No hay delivery ${daysList}` }));
      }
      return updated;
    });
  };
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">
            {blockedDeliveryDays.filter(d => d.blocked).length} día(s) bloqueado(s)
          </span>
        </div>
        
        <p className="text-sm text-gray-500 mb-4">
          Seleccioná qué días de la semana NO hay delivery. Los clientes no podrán elegir delivery en esos días.
        </p>

        {/* Dropdown para agregar días */}
        {blockedDeliveryDays.some(d => !d.blocked) && (
          <div className="flex items-center gap-2 mb-5">
            <select
              id="add-blocked-day"
              className="flex-1 px-3 py-2.5 bg-white border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400 focus:border-red-400 appearance-none cursor-pointer"
              defaultValue=""
              onChange={(e) => {
                const dayIndex = parseInt(e.target.value);
                if (!isNaN(dayIndex)) {
                  handleToggleBlockedDay(dayIndex);
                  e.target.value = '';
                }
              }}
            >
              <option value="" disabled>Elegir día para bloquear...</option>
              {blockedDeliveryDays
                .filter(d => !d.blocked)
                .map(d => (
                  <option key={d.day_index} value={d.day_index}>{d.day_name}</option>
                ))
              }
            </select>
            <span className="text-gray-400 text-xs whitespace-nowrap">Seleccioná y se agrega</span>
          </div>
        )}

        {/* Lista de días bloqueados */}
        {blockedDeliveryDays.filter(d => d.blocked).length > 0 ? (
          <div className="space-y-2 mb-6">
            {blockedDeliveryDays
              ...
              <div 
                key={day.day_index}
                className="border border-red-200 bg-red-50 rounded-lg p-3 transition-all hover:shadow-sm"
              >
                <div className="flex items-center gap-3">
                  <span className="text-lg">🚫</span>
                  <span className="font-semibold text-sm text-red-800 min-w-[90px]">{day.day_name}</span>
                  <input
                    type="text"
                    value={day.reason || ''}
                    onChange={(e) => handleUpdateDayReason(day.day_index, e.target.value)}
                    placeholder="Motivo (opcional)"
                    className="flex-1 px-2.5 py-1.5 bg-white border border-red-200 rounded-lg text-sm focus:ring-2 focus:ring-red-300 placeholder-gray-400"
                  />
                  <button
                    type="button"
                    onClick={() => handleToggleBlockedDay(day.day_index)}
                    className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-100 rounded-lg transition-colors"
                    title={`Habilitar delivery los ${day.day_name}`}
                  >
          ...
          <div className="text-center py-6 mb-6 bg-green-50 border border-green-200 rounded-lg">
            <span className="text-2xl block mb-1">✅</span>
            <p className="text-sm text-green-700 font-medium">Delivery habilitado todos los días</p>
            <p className="text-xs text-green-600 mt-1">Usá el selector de arriba para bloquear un día</p>
          </div>
        )}
